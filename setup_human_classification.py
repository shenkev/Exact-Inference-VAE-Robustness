...
with myFile:
    ...
    for i in range(imgs_to_check):
        ...
        row = [best_l2_img, 'hmc_l2', attack_set_labels[i], adversarial_targets[i], -1, new_filename, -1]
        writer.writerow(row)
        ...
        row = [mcmc_mean, 'mcmc_mean', attack_set_labels[i], adversarial_targets[i], -1, new_filename, -1]
        writer.writerow(row)
        ...
        row = [adv_img, 'adv', attack_set_labels[i], adversarial_targets[i], -1, new_filename, -1]
        writer.writerow(row)
