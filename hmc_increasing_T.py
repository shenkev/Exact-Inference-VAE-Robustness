...
from mcmc.mcmc2 import build_experiment, trim_32_to_28
...
model_eval.build()
model_eval.set_defaults(reconstruction={'sampling': model_sample_reconstructions})

model_eval.load(checkpoint)
...
def evaluate(qz_kept, num_samples, iter):
    ...
    vae_recon = trim_32_to_28(model_eval.decode_op(model.encode_op(x_ad)))
    ...
    l2_losses, recon_losses, l_latent_losses, x_samples, mean_imgs, vae_recon, vae_l2_loss, vae_recon_loss, vae_latent_loss \
        = sess.run(
        [l2_losses, recon_losses, l_latent_losses, x_samples, mean_imgs, vae_recon, vae_l2_loss, vae_recon_loss,
         vae_latent_loss], feed_dict={model_eval._training: False})
    ...
    for i in tqdm(range(inference_batch_size)):
        ...
        for j in range(sample_to_vis):
            plot_save(x_samples[j:j + 1, i], '{}_T={}_mcmc_sample_{}'.format(img_num, iter, j + 1))

        plot_save(mean_imgs[i:i + 1], '{}_T={}_mcmcMean'.format(img_num, iter))
        ...
        plot_save(vae_recon[i:i + 1], '{}_T={}_vae_recon'.format(img_num, iter))
        plot_save(best_recon_sample, '{}_T={}_best_recon'.format(img_num, iter))
        plot_save(best_l2_sample, '{}_T={}_best_l2'.format(img_num, iter))
        plot_save(best_latent_sample, '{}_T={}_best_latent'.format(img_num, iter))

        print("---------- Summary Image {} at T={} ------------".format(start_ind + i + 1, iter), file=f)
    ...
    return vae_recon_loss[i], vae_l2_loss[i], vae_latent_loss[i], best_recon_loss, best_l2_loss, best_latent_loss,\
           avg_recon_loss[i], avg_l2_loss[i], avg_latent_loss[i]

# =============================== INFERENCE ====================================
inference_batch_size = 1  # use a multiple of 32, I'm too lazy to hack around his reconstruct code which only takes mul of 32
# start_ind = int(os.environ['image_index'])  # don't exceed 448 for inference_batch_size or GPU runs out of memory on cluster
start_ind = 1
...
inference, qz = build_experiment(model.decode_op, model.encode_op, x_ad, config, model.discriminator_l_op)
...
for i in range(hmc_steps):
    ...
    if (i+1) % evaluation_interval == 0:
        ...
        to_keep = max(int(keep_ratio * evaluation_interval), num_samples)
        qz_kept = Empirical(qz.params[(i+1-to_keep): i+1])

        loss = [i+1]
        loss.extend(list(evaluate(qz_kept, num_samples, i+1)))
        losses.append(loss)

header = ['T', 'VAE recon loss', 'VAE L2 loss', 'VAE latent loss', 'Best mcmc recon loss', 'Best mcmc L2 loss',
          'Best mcmc latent loss', 'Average mcmc recon loss', 'Average mcmc l2 loss', 'Average mcmc latent loss']

myFile = open('./out/{}_loss.csv'.format(str(start_ind + 1).zfill(3)), 'w')
with myFile:
